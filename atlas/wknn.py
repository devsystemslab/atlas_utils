...
def build_nn(ref, query=None, k=100, use_gpu=False):
    if query is None:
        query = ref
...
def build_mutual_nn(dat1, dat2=None, k1=100, k2=None, use_gpu=False):
    if dat2 is None:
        dat2 = dat1
    if k2 is None:
        k2 = k1

    adj_12 = build_nn(dat1, dat2, k=k2, ue_gpu=use_gpu)
    adj_21 = build_nn(dat2, dat1, k=k1, ue_gpu=use_gpu)

    adj_mnn = adj_12.multiply(adj_21.T)
    return adj_mnn


def get_transition_prob_mat(dat, k=50, symm=True, use_gpu=False):
    adj = build_nn(dat, k=k, use_gpu=use_gpu)
    if symm:
...
def get_wknn(
    ...
    adj_r2q = None
    if ref2query:
        adj_r2q = build_nn(ref=query, query=ref, k=k, use_gpu=use_gpu)
    ...
    if ref2 is None:
        ref2 = ref
    adj_ref = build_nn(ref=ref2, k=k, use_gpu=use_gpu)
